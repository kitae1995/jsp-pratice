 ...
 * 비즈니스 로직이란, 외부 프로그램과	 연계 작업 등을 수행하는 로직입니다. ( 데이터 베이스 등,,,,)
 * 
 * 
 * VO(value object)
 ...
 * 규약 1번 : 자바빈은 은닉(캡슐화)를 사용하여 클래스를 구현함(변수 private , getter/setter)
 * 2.getter 메서드는 매개변수가 존재하지 않아야한다.
 * 3. 생성자는 매개값을 받지 않는 생성자(필수), 매개값을 받는 생성자가(선택)가 존재한다.
 * 4. 모든 getter/seteer 메서드는 접근제한이 pulbic이 이어햔다.
 */
